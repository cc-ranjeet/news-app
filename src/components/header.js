...
const Header = (props) => {
  ...
  const handleChange = event => {
    ...
    const filterByLanguage = event => {
      searchData.language = setSearchData({
  ...
  <Link className="navbar-brand" to="/"><img className="log-width" src={logo} alt="Logo" /></Link>
    ...
    <select className="form-select form-control me-2" aria-label="Default select example" name ="country" value={searchData.country} onChange={filter}>
        ...
        <input className="form-control me-2" type="search" name ="q" placeholder="Search" aria-label="Search"  onBlur={filter}/>
        <button className="btn btn-outline-success" type="button" onClick={handleChange}>Search</button>
      </form>
    </div>
    </div>
    </div>
